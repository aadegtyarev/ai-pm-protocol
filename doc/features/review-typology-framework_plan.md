...
- **Review-typology registry** (home decided in arch-review — likely `WORKFLOW.md` as a `### Review typology` subsection, mirroring `### Decision authority` single-sourcing; alternatively a `/pm-audit` section): the 5 types with cadence/depth/scope/deterministic-half/AI-half; the proportional new-code-gating + last-sweep-marker concept; the per-type det-vs-AI naming (ties to #211); the heavier types marked as later slices.
- `.claude/commands/pm-audit.md` — strengthen `## Technical quality (full scope only)` into the **smell/hygiene sweep**: proportional scope via the last-sweep marker, silent/skip gate, depth-selectable (drop hard-wired `ultra`), findings via the existing triage with `accepted (quality-sweep-<date>)`, autonomous procedural-gate bounded by proportionality, referencing the registry by name. Keep the interactive yes/no offer.
- `.claude/commands/pm-audit.md` (marker) — record/read the **last-sweep marker** (exact home — a line in the audit report vs a dedicated `.ai-pm/` file — decided in arch-review).
- `doc/architecture.md` — a short decision record: the review-typology framework as a new whole-system discipline (layered review, new-code gating, det-vs-AI split), EPIC slice 1, the registry home + marker home chosen, heavier types deferred. (pm-architect post-coding handoff.)
