...
> **STATUS: APPROVED (direction) — ready for coding.** Arch note `.ai-pm/arch/harness-neutral-prose_arch.md` settled the central fork; PM confirmed (AskUserQuestion 2026-06-07). Decisions: **scope = full corpus** (bodies + `WORKFLOW.md` + `workflow/*.md`); **form = A — genuinely-neutral prose on both harnesses + one reference table** (the Claude `.claude/` golden is **re-frozen**; the byte-identical guarantee is consciously spent — it is retired anyway post-extraction when `.claude/` becomes symlinks into dist). Accepted arch-note recommendations: route-reminder = name the intent neutrally, each adapter doc states its realized timing (per-prompt on Claude / always-on on OpenCode); install command-body prose = **deferred** to the install-auto-detect slice (allowlisted meanwhile); reference table = a generated `.md` rendered from `src/manifests/capabilities.json` (the single source).
