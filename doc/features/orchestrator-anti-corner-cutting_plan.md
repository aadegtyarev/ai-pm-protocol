...
### Transparency rider (Piece 0b — added 2026-06-08 from a live nula observation)
First live test of the core invariant on the freshly-bumped nula (`9b9ec93`, session `ses_157df89b9ffe`): the orchestrator behaved CORRECTLY — it read the prior `audit-2026-06-08.md` for CONTEXT, explicitly cited "gate integrity — fresh spawn this turn", and spawned a fresh `pm-auditor`. BUT the PM read it as "looks like it tried to corner-cut", because (a) it announced the read-for-context intent only AFTER reading, and (b) it RELAYED the old audit's results to the PM ("10 blocking, 6 notes") before spawning — edging toward presenting the stale artifact as the answer. The rule was followed but not VISIBLY followed. Fix — a transparency rider on the Delegation & gate integrity rule (persona + `workflow/pm-comms.md`): when reading an existing gate artifact (an `audit-*.md` / `_review.md` / plan) for context before a fresh spawn, (1) **announce the read-for-context-not-reuse intent BEFORE reading** ("I see a prior X — reading it for context only, not as the result; the gate needs a fresh spawn this turn"), and (2) **do NOT relay the old artifact's findings to the PM** as if they were the answer — the results the PM sees come from the fresh agent. Makes a legitimate context-read visibly distinct from an attempted corner-cut. Cross-harness (PM-comms + core rule + OpenCode persona echo). New prose-grep test. Buildable now (no spike).
