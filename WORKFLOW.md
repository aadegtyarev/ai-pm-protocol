...
After the bump, run `/pm-audit` — the same sweep that runs the mechanical pending-migration detection now also runs the semantic discipline sub-check (`### Expected-discipline manifest` in `MIGRATIONS.md`), so the documentation disciplines the new version introduced surface for the PM.
