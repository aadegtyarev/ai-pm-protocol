...
- Apply **deployment-context enrichment** per `### Deployment-context triage` in `workflow/review-typology.md` — before spawning the review subagent, read `docs/architecture.md` `## Operational limits & budgets` + `## Architectural constraints` (and `docs/threat-model.md` when present) and include them in the subagent prompt so the reviewer calibrates severity for the actual deployment target. Read the rule by name; do not re-encode it here.
