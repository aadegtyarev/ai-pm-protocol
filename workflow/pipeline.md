...
- After `code-review` completes, run the **seam-completeness angle** per `### Seam-completeness per-diff angle` in `workflow/review-typology.md` (a separate Agent with the 3-item checklist — exception-boundary, store-contract-consistency, symmetry-of-paranoia; findings merge into `## Code review findings`).
