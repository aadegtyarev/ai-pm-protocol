...
**Apply config** (the orchestrator's `## Setup` step 4): after `## Setup` writes `ai-pm.config.json`, the orchestrator re-assembles the agents so a chosen model takes effect — **`node adapter/claude/install-agents.mjs`** (the reviewer model is resolved at spawn here, so this just refreshes the assembled bodies; re-run `install-commands.mjs` too if the command surface changed). Because setup applies the config itself, the install/setup **order no longer matters** — running install before `/setup` no longer silently drops a pin (the apply step bakes the just-written config). It is idempotent: zero-config emits no model line, so the files come out unchanged. (`apply-config` in `tool-map.json` is the single-home record; the path is rewritten in a downstream, same as every wiring step above.)
...
**Live-verified on opencode 1.17.x:** on an unconfigured project a work request fired the `chat.message` inject and the nudge **reached the model** — the orchestrator offered `setup` instead of starting the task. From there the full `/setup` flow ran end-to-end: env discovery (`opencode models` → 9 models, session `deepseek/deepseek-v4-pro`), the structured-question dialog, the config **write** (`platform: opencode`, reviewer pin `{ "opencode": "deepseek/deepseek-v4-flash" }`), the orchestrator's apply/re-assemble step, AND the reviewer model-**pin bake** — the run baked `model: deepseek/deepseek-v4-flash` into the deployed `.opencode/agents/pm-reviewer.md`. Reconfigure was demonstrated too: setup read the existing config, showed a was/now diff, and rewrote + re-applied. The `install-commands.test.mjs` / `install-model.test.mjs` unit tests now stand alongside a full live run, not in place of one.
...
**Apply config** (the orchestrator's `## Setup` step 4): after `## Setup` writes `ai-pm.config.json`, the orchestrator re-assembles the agents so a chosen model takes effect — **`node adapter/opencode/install-agents.mjs`** (this is where OpenCode bakes the reviewer `model:` line into the assembled frontmatter, since there is no per-spawn model arg; re-run `install-commands.mjs` too if the command surface changed). Because setup applies the config itself, the install/setup **order no longer matters** — running install before `/setup` (the fresh-install path) no longer silently drops a pin chosen during setup, since the apply step re-bakes the just-written config. It is idempotent: `auto`/`session`/absent emit no model line, so the files come out unchanged. (`apply-config` in `tool-map.json` is the single-home record; the path is rewritten in a downstream, same as every wiring step above.)
