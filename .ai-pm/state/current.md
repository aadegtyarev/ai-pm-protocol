...
- **Pass-2 refinements (PM, 2026-06-07 — folded into the parked plan):** (a) **symmetric, no cross-read** — each harness reads only its own adapter, `.opencode/` self-contained (rejected OpenCode reading `.claude/skills/`); (b) **single source of truth, no duplicated bodies** — symlink where byte-identical, read-and-follow pointer (or OpenCode config `prompt:{file:}`) where frontmatter differs; **body duplication + drift-check fallback REJECTED** (self-maintained protocol can't safely keep two copies; "else simpler to make two repos") → the equivalence test is recast as a `single-source-integrity` guard; (c) **explicit declarative harness-support marker** in README + AGENTS.md (not inferred from folder presence); (d) `@`-inline **confirmed unavailable for agent bodies on both harnesses** (canonical docs) → no longer a spike; OpenCode command bodies do support `@file` (asymmetric, recorded). Still PARKED/UNAPPROVED, no coding.
- **Done (orchestrator):**
