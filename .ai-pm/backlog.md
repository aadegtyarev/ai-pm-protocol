...
PM-observed in the live nula OpenCode session (2026-06-08): the `ai-pm` orchestrator does NOT call `pm-architect` (and not all the other checking/support agents) when it should. **Diagnosis (orchestrator, confirmed):** NOT a deploy/loading problem — `opencode agent list` in nula shows `ai-pm` (primary) + all 10 `pm-*`/engine subagents loaded via the submodule symlinks; nula is on the s14 build (`3b83460`) whose persona already carries the strict step-by-step pipeline. The real root: several pipeline agents are invoked **conditionally**, and a weak model (DeepSeek) under-counts the condition and skips them —
...
- **Why not structural:** a plugin can deny an action (write), but cannot force a positive action (spawn agent X), so the backstop that worked for write-ownership (s15) does not transfer to delegation. The lever here is persona explicitness + possibly a pre-ship gate that refuses to proceed if a required step's artifact is missing (e.g. no plan-checker stamp). 
