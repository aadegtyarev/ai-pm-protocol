...
2. **For each component** — use the `deep-research` skill as your gathering + adversarial-verification engine, then extract the protocol-specific structure below. `deep-research` fans out across canonical docs, spec, and issue trackers and fact-checks claims — exactly what the "never quote from memory" rule needs. Keep `WebFetch` / `WebSearch` for targeted follow-up (a specific spec anchor, an exact validator command line). Do not assume from model memory. For each component capture:
   ...
   - **Seed from the library (read BEFORE researching new idioms).** Before researching new idioms for this stack component, check `doc/_templates/stack-idioms/<stack>.md` (where `<stack>` matches the language or framework being documented — e.g. `python.md`, `typescript.md`). Use the Read tool; the file is not @-imported. If the file exists, include the library's entries in your output as the starting set of idioms; treat them as already-verified patterns. Do not rediscover or re-document what the library already contains.
   - **Contribute-up recommendation (emit AFTER documenting new idioms).** For each new idiom you document that is not yet in the library file: if this is the FIRST time you are documenting this idiom for this project, annotate the entry with `[first occurrence — not yet promoted to library]`. If you are aware (from the plan or state context) that this idiom has already appeared on a prior project, emit a contribute-up recommendation: "Contribute-up candidate: `<idiom-name>` (`<stack>`) — recurs across projects; recommend adding to `doc/_templates/stack-idioms/<stack>.md`." The orchestrator decides whether to contribute; the researcher never writes to `doc/_templates/stack-idioms/` directly.
