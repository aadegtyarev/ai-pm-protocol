...
|---|---|
| **«хочу добавить фичу X»** | Mode = new-feature (implicit). Спроси: «Эта фича для существующей persona? Привязывается к шагу journey'я?» Если PM не уверен — quick read из `personas.md`/`user-journeys.md` + summary. Если фича требует новой persona/journey/threat — предложи **отдельный PR docs/<topic>** до feature branch. Иначе → handoff в Step 1 spec drafting. |
| **«починим баг X»** | Bug-fix variant. Создай `<topic>_spec.md` с frontmatter `lite-mode: bugfix`. Spec короткий: Context (что баг), Reproduction (failing test), Expected behavior, Fix scope. Дальше — обычный Step 2-7 (lite-режим в planner и reviewer). **Исключение:** bug в security path → full ceremony, no lite-mode. |
...
«Готов к написанию `.ai-pm/doc/features/<topic>_spec.md`. Подскажи topic и контекст; могу драфтить или PM сам напишет.»
